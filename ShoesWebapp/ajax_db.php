<?php
    include_once 'database.php';
    session_start();

        // จังหวัด
        if (isset($_POST['function']) && $_POST['function'] == 'provinces') {
            $id = $_POST['id'];
            $sql = "SELECT * FROM amphures WHERE province_id='$id'";
            $query = mysqli_query($conn, $sql);
            echo '<option style="color:black" value="" selected disabled>-กรุณาเลือกอำเภอ-</option>';
            foreach ($query as $value) {
                // ผลลัพธ์ออกมาเป็นตัวเลข
                echo '<option style="color:black" value="'.$value['id'].'">'.$value['name_th'].'</option>';
                
            }
        }
       
    //    อำเภอ
      if (isset($_POST['function']) && $_POST['function'] == 'amphures') {
          $id = $_POST['id'];
          $sql = "SELECT * FROM districts WHERE amphure_id='$id'";
          $query = mysqli_query($conn, $sql);
          echo '<option style="color:black" value="" selected disabled>-กรุณาเลือกตำบล-</option>';
          foreach ($query as $value2) {
              // ผลลัพธ์ออกมาเป็นตัวเลข
            echo '<option style="color:black" value="'.$value2['id'].'">'.$value2['name_th'].'</option>';
            
            
          }
        }
    //    ตำบล
        if (isset($_POST['function']) && $_POST['function'] == 'districts') {
          $id = $_POST['id'];
          $sql = "SELECT * FROM districts WHERE id='$id'";
          $query3 = mysqli_query($conn, $sql);
          $result = mysqli_fetch_assoc($query3);
          echo $result['zip_code'];
          exit();
        }
?>